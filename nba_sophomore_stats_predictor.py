...
player_names = dft['PLAYER'].values

# Extract features and targets
X_test = dft[['PTS_x', 'REB_x', 'AST_x', 'STL_x', 'BLK_x', 'TOV_x', 'FG_PCT_x', 'FG3_PCT_x', 'FT_PCT_x', 'MIN_x']].to_numpy()
y_test = dft[['PTS_y', 'REB_y', 'AST_y', 'STL_y', 'BLK_y', 'TOV_y', 'FG_PCT_y', 'FG3_PCT_y', 'FT_PCT_y', 'MIN_y']].to_numpy()
...
comparison_df = pd.DataFrame({
    'PLAYER': player_names,
    'Actual_PTS': y_test[:, 0],
    'Predicted_PTS': y_pred[:, 0],
    'Actual_REB': y_test[:, 1],
    'Predicted_REB': y_pred[:, 1],
    'Actual_AST': y_test[:, 2],
    'Predicted_AST': y_pred[:, 2],
    'Actual_STL': y_test[:, 3],
    'Predicted_STL': y_pred[:, 3],
    'Actual_BLK': y_test[:, 4],
    'Predicted_BLK': y_pred[:, 4],
    'Actual_TOV': y_test[:, 5],
    'Predicted_TOV': y_pred[:, 5],
    'Actual_FG_PCT': y_test[:, 6],
    'Predicted_FG_PCT': y_pred[:, 6],
    'Actual_FG3_PCT': y_test[:, 7],
    'Predicted_FG3_PCT': y_pred[:, 7],
    'Actual_FT_PCT': y_test[:, 8],
    'Predicted_FT_PCT': y_pred[:, 8],
    'Actual_MP': y_test[:, 9],
    'Predicted_MP': y_pred[:, 9],
})
predicted_cols = [col for col in comparison_df.columns if col.startswith('Predicted')]
comparison_df[predicted_cols] = comparison_df[predicted_cols].round(3)

# Output
print("=== Prediction Comparison ===")
print(comparison_df)
print("\n=== Model Error Metrics (All Players) ===")
print(f"Mean Absolute Error (MAE): {mae:.4f}")
print(f"Mean Squared Error (MSE): {mse:.4f}")

# MAE per stat and MAE % per stat
stat_names = ['PTS', 'REB', 'AST', 'STL', 'BLK', 'TOV', 'FG_PCT', 'FG3_PCT', 'FT_PCT', 'MIN']
mae_per_stat = {}
mae_percent_per_stat = {}

print("\n=== MAE per Stat ===")
for i, stat in enumerate(stat_names):
    mae_val = mean_absolute_error(y_test[:, i], y_pred[:, i])
    mean_val = np.mean(y_test[:, i])
    percent_error = (mae_val / mean_val) * 100 if mean_val != 0 else 0
    mae_per_stat[stat] = round(mae_val, 4)
    mae_percent_per_stat[stat] = round(percent_error, 2)
    print(f"{stat}: MAE = {mae_val:.4f}, MAE% = {percent_error:.2f}%")